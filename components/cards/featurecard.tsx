...
const FeatureCard: React.FC<FeatureCardProps> = ({ title, price, rating }) => {
  ...
  const renderDogPaws = (count: number) => {
    ...
    for (let i = 0; i < count; i += 1) {
      dogPaws.push(<DogPaw isFill key={i} />);
    }
  ...
  return (
    <div className="flex flex-col items-center justify-center">
      <div className="text-center">
        ...
        <h2 className="mt-2 text-[#333333] font-medium text-center">{title}</h2>
        ...
        <button
          type="button"
          className="bg-[#E2081E] text-[#FEFEFE] mt-2 pl-6 pr-6 text-sm py-1 px-4 rounded inline-flex gap-4 items-center"
        >
          <svg
